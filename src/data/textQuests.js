...
export const TEXT_QUESTS = [
    ...
    nodes: {
      ...
      level2: {
        ...
        options: [
          ...
          { text: '> Проверить трещину в стене', next: 'gas_pocket' },
  ...
  // ===== ЭХОЛОТ 4/4: ЯДРО ИСТИНЫ (Kernel, Глитч) =====
  {
    ...
    nodes: {
      ...
      read_code: {
        text: 'Код течёт перед глазами. Не символы - структуры. Паттерны, которые ваш мозг не должен понимать, но почему-то понимает.\n\nФункции создания материи. Циклы обработки времени. Массивы координат, уходящие в бесконечность.\n\nИ комментарий. Один. Человеческим языком, вплетённый в нечеловеческий код:\n\n"Для того, кто дочитал до сюда - мы знали, что вы придёте. Продолжайте."',
        sound: 'echoReveal',
        options: [
          { text: 'Приблизиться к Ядру', next: 'approach_core' },
          { text: 'Запомнить код и отступить', next: 'end_code_knowledge' },
        ]
      },
      end_code_knowledge: {
        text: 'Вы отступаете с фрагментами кода в памяти. Не полная картина - но достаточно, чтобы изменить ваше понимание реальности навсегда. Каспер был прав. Лира была права. Всё связано.',
        ending: true,
        result: { credits: 2000, reputation: 15, flags: { echolocator_code_read: true }, factionRep: { scientists: 8 } }
      },
      no_crystal: {
